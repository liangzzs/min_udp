...
udp_server::udp_server(const std::string &addr, int port) : f_port(port), f_addr(addr)
{
    ...
    {
        throw udp_client_server_runtime_error(
            ("invalid address or port for UDP socket: \"" + addr + ":" + decimal_port + "\"")
                .c_str());
    }
    f_socket = socket(f_addrinfo->ai_family, SOCK_DGRAM | SOCK_CLOEXEC, IPPROTO_UDP);
    if (f_socket == -1)
...
//     // int retval = select(f_socket, &s, &s, &s, &timeout);
//     std::cout << "sec = " << timeout.tv_sec << std::endl;
//     std::cout << "usec = " << timeout.tv_usec << std::endl;
